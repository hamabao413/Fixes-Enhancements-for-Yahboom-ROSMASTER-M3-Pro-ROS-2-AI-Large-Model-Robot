...
class color_detect:
    ...
    def object_follow(self, img, hsv_msg):
        ...
        lower = np.array(hsv_msg[0], dtype="uint8")
        upper = np.array(hsv_msg[1], dtype="uint8")
        # 根据特定颜色范围创建mask（支援 Hue wrap-around，例如 170..10）
        # Create a mask based on a specific color range (support Hue wrap-around)
        h_lo = int(lower[0])
        h_hi = int(upper[0])
        if h_lo <= h_hi:
            mask = cv.inRange(src, lower, upper)
        else:
            # Hue 跨越 0：拆成兩段 [0..h_hi] U [h_lo..179]
            lower1 = np.array([0, int(lower[1]), int(lower[2])], dtype="uint8")
            upper1 = np.array([h_hi, int(upper[1]), int(upper[2])], dtype="uint8")
            lower2 = np.array([h_lo, int(lower[1]), int(lower[2])], dtype="uint8")
            upper2 = np.array([179, int(upper[1]), int(upper[2])], dtype="uint8")
            mask = cv.bitwise_or(cv.inRange(src, lower1, upper1), cv.inRange(src, lower2, upper2))
        color_mask = cv.bitwise_and(src, src, mask=mask)
        # 将图像转为灰度图
        # Convert the image to grayscale
        gray_img = cv.cvtColor(color_mask, cv.COLOR_RGB2GRAY)
    ...
    def Roi_hsv(self, img, Roi):
        ...
        修正要点（針對紅/黃誤判、遮罩整片白）：
        1) OpenCV HSV Hue 範圍是 0..179（不是 0..255）
        2) ROI 若包含白色本體/反光/桌面，S_min/V_min 會被拉太低，導致把白色大量吃進遮罩
        3) 本方法改採「有效像素（較高飽和/亮度）」做統計，並用主峰收斂 Hue 範圍
        4) 紅色可能跨越 Hue=0（例如 170..10），會回傳 H_min > H_max；需搭配 inRange 的 wrap-around 支援（本補丁已在 object_follow 修正）
        '''
        ...
        x1 = 0 if x1 < 0 else (w_img - 1 if x1 >= w_img else x1)
        x2 = 0 if x2 < 0 else (w_img - 1 if x2 >= w_img else x2)
        y1 = 0 if y1 < 0 else (h_img - 1 if y1 >= h_img else y1)
        y2 = 0 if y2 < 0 else (h_img - 1 if y2 >= h_img else y2)
        ...
        roi = HSV[y1:y2, x1:x2, :]
        H = roi[:, :, 0].astype(np.uint8).reshape(-1)
        S = roi[:, :, 1].astype(np.uint8).reshape(-1)
        V = roi[:, :, 2].astype(np.uint8).reshape(-1)

        # 過濾低飽和/低亮度（白色本體、反光、桌面通常在這裡）
        valid = (S >= 70) & (V >= 70)
        if int(np.count_nonzero(valid)) >= 80:
            ...
            # fallback：有效像素太少就用原始 ROI
            H2, S2, V2 = H, S, V

        # Hue：用主峰為中心縮窄（紅/黃相鄰特別需要）
        hist = cv.calcHist([H2.reshape(-1, 1)], [0], None, [180], [0, 180])
        peak = int(hist.argmax()) if hist is not None else int(np.median(H2))

        # 預設半寬（想更嚴格可調小，例如 14）
        w = 16

        # 紅色靠近 0/179，允許 wrap-around
        if peak <= 12 or peak >= 168:
            H_min = (peak - w) % 180
            H_max = (peak + w) % 180
        else:
            H_min = peak - w
        ...
        # S/V：取下界分位數，再往下放寬一點；並保底避免吃到白
        try:
            sp10 = float(np.percentile(S2, 10))
            vp10 = float(np.percentile(V2, 10))
        except Exception:
            sp10 = float(np.min(S2))
            vp10 = float(np.min(V2))

        S_min = int(round(sp10)) - 20
        V_min = int(round(vp10)) - 20
        if S_min < 0: S_min = 0
        if V_min < 0: V_min = 0

        # 保底門檻：避免把白色/灰色大量吃進來（紅/黃對反光更敏感）
        if S_min < 60: S_min = 60
        if V_min < 60: V_min = 60

        S_max = 255
        V_max = 255

        lowerb = 'lowerb : (' + str(int(H_min)) + ' ,' + str(int(S_min)) + ' ,' + str(int(V_min)) + ')'
        upperb = 'upperb : (' + str(int(H_max)) + ' ,' + str(int(S_max)) + ' ,' + str(int(V_max)) + ')'
        txt2 = 'OK !!!'
