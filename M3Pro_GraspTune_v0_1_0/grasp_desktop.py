...
class GraspNode(Node):
	def __init__(self, name):
		...
		# === Gripper / Grasp tuning (unit: meter / degree) ===
		# 1) 夾不到(太後面) -> 增加 grasp_bias_x (每次 +0.005 約 5mm)
		# 2) 夾不到(太高/太低) -> 微調 grasp_bias_z (每次 0.002~0.005)
		# 3) 夾不緊/掉落 -> 增加 gripper_close_angle (建議 130~170)
		self.gripper_open_angle = 30
		self.gripper_close_angle = 145
		self.grasp_bias_x = 0.010
		self.grasp_bias_y = 0.010
		self.grasp_bias_z = -0.005
	...
	def get_joint6_callback(self,msg):
		# set_joint6：當作「夾爪關閉角度」覆寫值（degree）
		self.joint6 = int(msg.data)
		self.gripper_close_angle = int(msg.data)
		print("get joint6 (gripper_close_angle) = ", int(msg.data))
	...
	def grasp(self,pose_T):
		...
		request.tar_y = pose_T[1] + self.grasp_bias_y
		request.tar_z = pose_T[2] + self.grasp_bias_z  # negative -> down a bit
		request.tar_x = pose_T[0] + self.grasp_bias_x
		print("[tune] grasp_bias(m)=", (self.grasp_bias_x, self.grasp_bias_y, self.grasp_bias_z), " open/close=", (self.gripper_open_angle, self.gripper_close_angle))
	...
	def get_ik_respone_callback(self, future):
		try:
			...
			joints[5] = int(self.gripper_open_angle)
	...
	def move(self):
		...
		#time.sleep(2.5)
		self.pubSingleArm(6, int(self.gripper_close_angle), 2000)
		time.sleep(2.5)
		self.pubSingleArm(2, 120, 2000)
		time.sleep(2.5)
		if self.cur_tagId == 1:
			self.down_joint = [180, 35, 70, 5, 60, int(self.gripper_close_angle)]
		elif self.cur_tagId == 2:
			self.down_joint = [180, 35, 60, 0, 90, int(self.gripper_close_angle)]
		elif self.cur_tagId == 3:
			self.down_joint = [0, 30, 70, 2, 90, int(self.gripper_close_angle)]
		elif self.cur_tagId == 4:
			self.down_joint = [0, 43, 48, 6, 90, int(self.gripper_close_angle)]
		self.pubSixArm(self.down_joint)
		time.sleep(2.5)
		self.pubSingleArm(6, int(self.gripper_open_angle), 2000)
		time.sleep(2.5)
